...
(function (factory) {
    ...
    $h = {
        ...
        dataURI2Blob: function (dataURI) {
            var BlobBuilder = window.BlobBuilder || window.WebKitBlobBuilder || window.MozBlobBuilder || window.MSBlobBuilder,
                canBlob = $h.hasBlobSupport(), byteStr, arrayBuffer, intArray, i, mimeStr, bb,
    ...
    FileInput.prototype = {
        ...
        _initResumableUpload: function () {
            ...
            self.resumableManager = {
                init: function (id, f, index) {
                    ...
                    var rm = self.resumableManager, fm = self.fileManager, id = rm.id, msg, $thumb = rm.$thumb,
                        $prog = rm.$progress, hasThumb = $thumb && $thumb.length,
                    ...
                    var rm = self.resumableManager, opts = self.resumableUploadOptions, fd, f, fm = self.fileManager,
                        id = rm.id, fnBefore, fnSuccess, fnError, fnComplete, outData;
                    if (!opts.testUrl) {
                        rm.testing = false;
                        return;
                    }
                    ...
                    var fd, outData, fnBefore, fnSuccess, fnError, fnComplete,
                    ...
                    fnSuccess = function (data, textStatus, jqXHR) {
                        ...
                        if (data.error) {
                            if (opts.showErrorLog) {
                                self._log(logs.retryStatus, {retry: retry + 1, filename: rm.fileName, chunk: index});
                            }
        ...
        _initTemplateDefaults: function () {
            ...
            tModalMain = '<div id="' + $h.MODAL_ID + '" class="file-zoom-dialog modal fade" ' + 'tabindex="-1" aria-labelledby="' + $h.MODAL_ID + 'Label"></div>';
            tModal = '<div class="modal-dialog modal-lg{rtl}" role="document">\n' + '  <div class="modal-content">\n' + '    <div class="modal-header">\n' + '      <h5 class="modal-title">{heading}</h5>\n' + '      <span class="kv-zoom-title"></span>\n' + '      <div class="kv-zoom-actions">{toggleheader}{fullscreen}{borderless}{close}</div>\n' + '    </div>\n' + '    <div class="modal-body">\n' + '      <div class="floating-buttons"></div>\n' + '      <div class="kv-zoom-body file-zoom-content {zoomFrameClass}"></div>\n' + '{prev} {next}\n' + '    </div>\n' + '  </div>\n' + '</div>\n';
            tProgress = '<div class="progress">\n' + '    <div class="{class}" role="progressbar"' + ' aria-valuenow="{percent}" aria-valuemin="0" aria-valuemax="100" style="width:{percent}%;">\n' + '        {status}\n' + '     </div>\n' + '</div>{stats}';
            tStats = '<div class="text-info file-upload-stats">' + '<span class="pending-time">{pendingTime}</span> ' + '<span class="upload-speed">{uploadSpeed}</span>' + '</div>';
            ...
            tImage = '<img src="{data}" class="file-preview-image kv-preview-data" title="{title}" ' + 'alt=""' + tStyle + '>\n';
            ...
            self.defaults = {
                ...
                previewMarkupTags: {tagBefore1: tTagBef1, tagBefore2: tTagBef2, tagAfter: tTagAft},
                ...
                fileTypeSettings: {
                    image: function (vType, vName) {
                        ...
                        return $h.compare(vType, 'text.*') || $h.compare(vName, /\.(xml|javascript)$/i) || $h.compare(vName, /\.(txt|md|csv|nfo|ini|json|php|js|css)$/i);
                ...
                data: {}, init: function () {
                    ...
                    var ind = 'init_' + i, data = self.previewCache.data, config = data.config[i], fileId,
                        ...
                        parseTemplate = function (cat, dat, fn, ft, id, ftr, ind, fc, t) {
                            ...
                            return self._generatePreviewTemplate(cat, dat, fn, ft, id, fileId, false, null, fc, ftr, ind, t, a, config && config.zoomData || dat);
                        };
                    ...
                    if (asData) {
                        ...
                        out = parseTemplate('generic', content, fname, ftype, previewId, ftr, ind, frameClass, cat).setTokens({'content': data.content[i]});
                    }
                    ...
                    var config = data.config[i], caption = $h.ifSet('caption', config), a,
                        ...
                        dUrl = config.downloadUrl || downloadInitialUrl, dFil = config.filename || config.caption || '',
        ...
        _isPdfRendered: function () {
            var self = this, useLib = self.usePdfRenderer, flag = typeof useLib === 'function' ? useLib() : !!useLib;
        ...
        _listenModalEvent: function (event) {
            var self = this, $modal = self.$modal, getParams = function (e) {
                return {sourceEvent: e, previewId: $modal.data('previewId'), modal: $modal};
            };
        ...
        _resetUpload: function () {
            ...
            if (self.overwriteInitial) {
                ...
                self.initialPreviewConfig = [];
                self.initialPreviewThumbTags = [];
                self.previewCache.data = {content: [], config: [], tags: []};
            }
        },
        ...
        _uploadBatch: function () {
            var self = this, fm = self.fileManager, total = fm.total(), params = {}, fnBefore, fnSuccess, fnError,
                fnComplete, hasPostData = total > 0 || !$.isEmptyObject(self.uploadExtraData), errMsg, setAllUploaded,
                formdata = new FormData(), op = self.ajaxOperations.uploadBatch;
            ...
            self.getFrames(' .kv-file-remove').each(function () {
                ...
                self._handler($el, 'click', function () {
                    ...
                    $frame.fadeOut('slow', function () {
                        ...
                        self._clearFileInput();
                        var chk = self.previewCache.count(true), len = self.fileManager.count(), file,
                            hasThumb = self.showPreview && self.getFrames().length;
        ...
        _initPreviewActions: function () {
            var self = this, $preview = self.$preview, deleteExtraData = self.deleteExtraData || {},
                ...
                origClass = settings.removeClass, errClass = settings.removeErrorClass, resetProgress = function () {
            ...
            $preview.find(btnRemove).each(function () {
                ...
                var $frame = $el.closest($h.FRAMES), cache = self.previewCache.data, settings, params, config, fileName,
                    extraData, index = $frame.attr('data-fileindex');
        ...
        _generatePreviewTemplate: function (cat, data, fname, ftype, previewId, fileId, isError, size, frameClass, foot, ind, templ, attrs, zoomData) {
        ...
        _setInitThumbAttr: function () {
            var self = this, data = self.previewCache.data, len = self.previewCache.count(true), config, caption, size,
                previewId;
            if (len === 0) {
                return;
            }
        ...
        _setThumbStatus: function ($thumb, status) {
            ...
            var icon = 'indicator' + status, msg = icon + 'Title', css = 'file-preview-' + status.toLowerCase(),
        ...
        _setFileDropZoneTitle: function () {
            ...
            if (!self.showPreview || $zone.length === 0 || self.fileManager.count() > 0 || !self.dropZoneEnabled || (!self.isAjaxUpload && self.$element.files)) {
                return;
            }
        ...
        _getStats: function (stats) {
            ...
            pendingTime = (!stats.elapsed || !stats.bps) ? self.msgCalculatingTime : self.msgPendingTime.setTokens({time: $h.getElapsed(Math.ceil(stats.pendingBytes / stats.bps))});
            return t.setTokens({uploadSpeed: stats.bitrate, pendingTime: pendingTime});
        },
        ...
        _setFileUploadStats: function (id, pct, total, stats) {
            ...
            var fm = self.fileManager, $thumb = fm.getThumb(id), pctTot, rm = self.resumableManager, totUpSize = 0,
                totSize = fm.getTotalSize(), totStats = $.extend(true, {}, stats);
            if (self.enableResumableUpload) {
        ...
        _validateImageOrientation: function ($img, file, previewId, fileId, caption, ftype, fsize, iData) {
            ...
            value = exifObj ? exifObj['0th'][piexif.ImageIFD.Orientation] : null;
            if (!value) {
        ...
        _initClickable: function () {
            ...
            self._handler($zone, 'click', function (e) {
                var $tar = $(e.target);
                if (!$(self.elErrorContainer + ':visible').length && (!$tar.parents('.file-preview-thumbnails').length || $tar.parents('.file-default-preview').length)) {
        ...
        _renderMain: function () {
            var self = this, dropCss = self.dropZoneEnabled ? ' file-drop-zone' : 'file-drop-disabled',
        ...
        _renderFileActions: function (cfg, showUpl, showDwn, showDel, showZoom, showDrag, disabled, url, key, isInit, dUrl, dFile) {
            var self = this;
            if (!cfg.type && isInit) {
                cfg.type = 'image';
            }
        ...
        _change: function (e) {
            ...
            var $el = self.$element, isDragDrop = arguments.length > 1, isAjaxUpload = self.isAjaxUpload, tfiles,
                files = isDragDrop ? arguments[1] : $el.get(0).files, total,
                maxCount = !isAjaxUpload && $h.isEmpty($el.attr('multiple')) ? 1 : self.maxFileCount, len,
                ctr = self.fileManager.count(), isSingleUpload = $h.isEmpty($el.attr('multiple')),
                flagSingle = (isSingleUpload && ctr > 0), throwError = function (mesg, file, previewId, index) {
        ...
        readFiles: function (files) {
            ...
            readFile = function (i) {
                ...
                var node = ctr + i, previewId = previewInitId + '-' + node, file = files[i], fSizeKB, j, msg, $thumb,
                    ...
                    caption = self._getFileName(file, ''), fileSize = (file && file.size || 0) / 1000, fileExtExpr = '',
                    previewData = $h.createObjectURL(file), fileCount = 0, strTypes = '', fileId, func, knownTypes = 0,
        ...
        resume: function () {
            ...
            if (self.paused) {
                $pr.html(self.progressPauseTemplate.setTokens({percent: 101, status: self.msgUploadResume, stats: ''}));
            } else {
                flag = true;
            }
            self.paused = false;
            if (flag) {
                $pr.html(self.progressInfoTemplate.setTokens({percent: 101, status: self.msgUploadBegin, stats: ''}));
            }
            setTimeout(function () {
                rm.upload();
            }, self.processDelay);
        ...
        disable: function () {
            ...
            self.$container.find('.fileinput-remove, .fileinput-upload, .file-preview-frame button').attr('disabled', true);
        ...
        enable: function () {
            ...
            self.$container.find('.fileinput-remove, .fileinput-upload, .file-preview-frame button').removeAttr('disabled');
    ...
    $.fn.fileinput.defaults = {
        ...
        autoOrientImage: function () {
            var ua = window.navigator.userAgent, webkit = !!ua.match(/WebKit/i), iOS = !!ua.match(/iP(od|ad|hone)/i),
    ...
    $.fn.fileinputLocales.zh = {
        ...
        msgPendingTime: '{time} remaining',
        msgCalculatingTime: 'calculating time remaining',
        ajaxOperations: {deleteThumb: '删除文件', uploadThumb: '上传文件', uploadBatch: '批量上传', uploadExtra: '表单数据上传'},
