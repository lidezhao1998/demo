 ...
 */
@Controller
@RequestMapping("/system/poison")
public class TaskPoisonController extends BaseController {
    private String prefix = "system/poison";
    ...
    @RequiresPermissions("system:poison:view")
    @GetMapping()
    public String report() {
        return prefix + "/poison";
    }

    /**
     * 查询黑土滩、毒害草、已垦草原治理报表
     */
    @RequiresPermissions("system:poison:list")
    //@GetMapping("/list")
    @PostMapping("/list")
    @ResponseBody
                ...
                cityDistinct.forEach(address -> {
                    ...
                    yearRange.forEach(year -> {
                        double sum = Double.parseDouble(df.format(listCityData.stream()
                                ...
                                .mapToDouble(TTaskPublish::getDhcSize).sum()));
        ...
        areaDistinct.forEach(address -> {
            ...
            yearRange.forEach(year -> {
                double sum = Double.parseDouble(df.format(listAreaData.stream()
                        .filter(item -> item.getYear().equals(String.valueOf(year)) && item.getAddress().equals(address))
                        .mapToDouble(TTaskResolve::getDhcSize).sum()));
                map.put(String.valueOf(year), String.valueOf(sum));
            });
            String province = address;
            map.put("province", province);
