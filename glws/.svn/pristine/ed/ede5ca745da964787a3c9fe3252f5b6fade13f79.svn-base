            ...
            if (idsThematicLayer != undefined && idsThematicLayer != null && idsThematicLayer != '') {
                $.post(prefixGis + "/getSpot",
                    {shpIds: idsThematicLayer}, function (res) {
                            var type="project_type in(" + res.featuresData + ") and create_by in(" + res.userIds + ")";
...
$("#special").click(function(){
    let options = {
        id:"reportTable",
        url: reportPrefix + "/selectGisListReport",
        ...
        columns: [
            [
                ...
                },
                {
                    ...
                    formatter: function (value, row, index) {
                        var actions = [];
                        actions.push('<a class="Change state btn btn-primary btn-xs" onclick="openReportDetail(\'' + row.reportId + '\')"><i class="fa fa-edit"></i>查看详情</a> ');
                        return actions.join('');
                    }
                },
            ],
...
function openReportDetail(id) {
    let url = prefixGis + "/detail/" + id;
    $.modal.openFul("上报信息详情",url)
}
function show3D() {
    $("#map").hide();
