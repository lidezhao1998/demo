...
var cityUrlData = "/gis/ows?service=WFS&version=1.0.0&request=GetFeature&maxFeatures=50&outputFormat=application/json&typeName=gis:grasslandecology&";
var gisMapName = "gis:sinosoft-gisMap";
var gisTabName = "gis:sinosoft-gisTab";
...
function pointQuery(type) {
    ...
    var pointQuery = map.on('click', function (evt) {
        ...
        if (queryType == "service") {
            ...
            var generic_code = "generic_code in (" + serviceIdList + ")";
            var bundle_code = "or bundle_code in (" + serviceIdList + ")";
            ...
            var idsThematicLayer = $.tree.getGisCheckedNodes("thematicLayer");
            if (idsThematicLayer == null || idsThematicLayer == "") {
                layer.msg("请选择业务图层", {icon: 6, time: 3000});
                return;
            }
            tiled = new ol.layer.Tile({
        ...
        $.getJSON(url, function (data) {
            ...
            for (var i = 0; i < 1; i++) {
                ...
                var cql = "";
                var lay=pointQueryGisGrass;
                if (queryType == "service") {
                    cql="id=" + jsonstr.id;
                    lay=pointQueryGisGrass;
                    ...
                    $.modal.open("查看业务数据草地信息", prefixPostGis + "/getGrassById?id=" + jsonstr.id, 400, 600);
                } else {
                    var mapId=parseInt(feature[i].id.replace(/[^0-9]/ig,""));
                    cql=mapId;
                    lay=gisMapName;
                    $.modal.open("查看专题数据草地信息", prefixGisMap + "/getGisMapById?id=" + mapId, 400, 550);
                    pointQueryTitleSource = new ol.layer.Tile({
                        source: new ol.source.TileWMS({
                            url: gisUrl + urlLayer,
                            params: {
...
function queryGisMap(id, type, geo, centre) {
    map.removeEventListener("dblclick", addGisMap);
    if (type == 1) {
        polygonArea = 0;
    }
    $.modal.open("查看专题数据草地信息", prefixGisMap + "/addGisMap?id=" + id + "&type=" + type + "&geo=" + geo + "&centre=" + centre + "&area=" + polygonArea, 400, 550);
}
/**
 * 上报绘图结束调用此方法
 * @param id
    ...
    var pointQuery = map.on('click', function (evt) {
        ...
        $.getJSON(url, function (data) {
            ...
            if (feature == [] || feature.length == 0 || feature == null) {
                ...
                var mapId=parseInt(feature[0].id.replace(/[^0-9]/ig,""));
                $.modal.open("查看专题数据草地信息", prefixGisMap + "/getGisMapById?id=" + mapId, 400, 550);
            }
        })
    })
