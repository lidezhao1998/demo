...
public class DynamicDataSourceRegister implements ImportBeanDefinitionRegistrar, EnvironmentAware {
    ...
    @Value("${spring.datasource.master.maxEvictableIdleTimeMillis}")
    ...
    public void registerBeanDefinitions(AnnotationMetadata annotationMetadata, BeanDefinitionRegistry beanDefinitionRegistry) {
        ...
        // 绑定默认数据源参数 也就是主数据源
        DataSource consumerDatasource, defaultDatasource =null;
        try {
            defaultDatasource = bind(clazz, defauleDataSourceProperties);
            consumerDatasource= defaultDatasource;
        }catch (Exception e){
            e.printStackTrace();
        }
    ...
    private <T extends DataSource> T bind(Class<T> clazz, Map properties) throws IllegalAccessException, InstantiationException, SQLException {
//        ConfigurationPropertySource source = new MapConfigurationPropertySource(properties);
//        Binder binder = new Binder(new ConfigurationPropertySource[]{
//                source.withAliases(aliases)
//        });
//        DruidDataSource dds= (DruidDataSource) clazz.newInstance();
//        dds.setMinEvictableIdleTimeMillis(Long.parseLong(properties.get("minEvictableIdleTimeMillis").toString()));
//        System.out.println("------"+Long.parseLong(properties.get("minEvictableIdleTimeMillis").toString()));
//        dds.setMaxEvictableIdleTimeMillis(Long.parseLong(properties.get("maxEvictableIdleTimeMillis").toString()));
//        System.out.println("------"+Long.parseLong(properties.get("maxEvictableIdleTimeMillis").toString()));
//
//        T t = null;
//        try {
//            t=(T)binder.bind(ConfigurationPropertyName.EMPTY, Bindable.of(dds.getClass())).get();
//        }catch (Exception e){
//            e.printStackTrace();
//        }
//        // 通过类型绑定参数并获得实例对象
