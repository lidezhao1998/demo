package com.sinosoft.web.controller.tool;
...
public class MathUtil {
    public static void main(String[] args) {
        ...
        double[] y = {1.0,2.0,3.0,5.0};

        double[] reslt=dxsFitting(x,y,1);
     ...
     *
     * @param x
     * @param y reuslt[0] = a result[1] = b result[2] 相关系数 result[3] 决定系数
     ...
     *
     * @param x
     * @param y result[0] = a result[b] = b . . . result[len - 4] 点数 result[len -
    ...
    /**
     * 参考网址：http://wenku.addressCity.com/link?url=T-xy9CUR_hVlCpA5o6xjqWb-Z1o5jraGEg-OHEtDgFLEynf6HedV68wMwC5u7RTs7PN5kiYq3qjyiddMveDFywLiuXai11MisNrNvv4KjAW&qq-pf-to=pcqq.c2c
     * 峰拟合 y = y(max) * exp[-(x - x(max))^2/S]
     *
     * @param x
     * @param y result[0] = x(max) result[1] = y(max) result[2] = S result[3] 点数
     ...
     * @param x
     * @param y
     * @param sf 算法 <input type="checkbox" value="0" />常数
     ...
     *
     * @param x
     * @paramy result[0] = x(max) result[1] = y(max) result[2] = S
     * @return
     */
     ...
     * 多元线性回归分析
     *
     * @paramx[m][n] 每一列存放m个自变量的观察值
     * @paramy[n]    存放随即变量y的n个观察值
     * @param m       自变量的个数
     * @param n       观察数据的组数
     * @param a       返回回归系数a0,...,am
     * @paramdt[4]   dt[0]偏差平方和q,dt[1] 平均标准偏差s dt[2]返回复相关系数r dt[3]返回回归平方和u
     * @paramv[m]    返回m个自变量的偏相关系数
     */
