...
(function ($) {
	$.extend({
		...
		businessLayerTree: {},
		thematicLayerTree: {},
		businessLayerTree_left: {},
		thematicLayerTree_right: {},
		businessLayerTree_right: {},
		thematicLayerTree_left: {},
		...
		modal: {
			...
			// 弹出层全屏
			openFul: function (title, url, width, height) {
				//如果是移动端，就使用自适应大小弹窗
				if (navigator.userAgent.match(/(iPhone|iPod|Android|ios)/i)) {
					width = 'auto';
					height = 'auto';
		...
		operate: {
			...
			// 详细信息
			detailful: function(id, width, height) {
				table.set();
				var url = "/404.html";
				...
				$.modal.openFull( table.options.modalName, url);
			},
			// 详细访问地址
			...
			add: function(id) {
				table.set();
				$.modal.open( table.options.modalName, $.operate.addUrl(id));
			},
			// 添加信息，以tab页展现
			addTab: function (id) {
				table.set();
				$.modal.openTab( table.options.modalName, $.operate.addUrl(id));
			},
			// 添加信息 全屏
			addFull: function(id) {
				table.set();
			...
			addFul: function(id) {
				table.set();
				var url = $.common.isEmpty(id) ? table.options.createUrl : table.options.createUrl.replace("{id}", id);
				$.modal.openFul( table.options.modalName, url);
			},
			// 查看驳回原因
			readFul: function(id) {
				table.set();
		...
		tree: {
			...
			init: function(options) {
				...
				$.get(options.url, function(data) {
					...
					$._tree = tree;
					if(options.id == "thematicLayer"){
						$.thematicLayerTree = tree;
					}else if(options.id == "businessLayer"){
						$.businessLayerTree = tree;
					}else if(options.id == "thematicLayer_left"){
						$.thematicLayerTree_left = tree;
					}else if(options.id == "thematicLayer_right"){
						$.thematicLayerTree_right = tree;
					}else if(options.id == "businessLayer_left"){
						$.businessLayerTree_left = tree;
					}else if(options.id == "businessLayer_right"){
						$.businessLayerTree_right = tree;
					}
			...
			// 获取当前业务树被勾选集合
			getGisCheckedNodes: function(treeId) {
				var nodes=""
				if(treeId=="businessLayer"){
					nodes = $.businessLayerTree.getCheckedNodes(true);
				}
				if(treeId=="thematicLayer"){
					nodes = $.thematicLayerTree.getCheckedNodes(true);
				}
				if(treeId=="businessLayer_left"){
					nodes = $.businessLayerTree_left.getCheckedNodes(true);
				}
				if(treeId=="thematicLayer_left"){
					nodes = $.thematicLayerTree_left.getCheckedNodes(true);
				}
				if(treeId=="businessLayer_right"){
					nodes = $.businessLayerTree_right.getCheckedNodes(true);
				}
				if(treeId=="thematicLayer_right"){
					nodes = $.thematicLayerTree_right.getCheckedNodes(true);
				}
				if(treeId.indexOf("b")==0){
					return $.map(nodes, function (row) {
						var newRow;
			...
			// 不允许根、父节点选择
			notAllowParentsFather: function(_tree) {
				var nodes = _tree.getSelectedNodes();
				if(nodes.length == 0){
					$.modal.msgError("请选择节点后提交");
					return false;
